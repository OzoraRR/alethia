...
export const practiceEventTypes = ["module_started", "stage_viewed", "sender_inspected", "link_inspected", "official_channel_verified", "decision_selected", "attacker_pov_viewed", "feedback_viewed", "retry_started", "module_completed", "locale_changed"] as const;
...
export async function startPracticeAttempt(moduleId: ModuleId = "courier-sms"): Promise<PracticeAttemptResult> {
  ...
  const { data, error } = await auth.client.from("practice_attempts").insert({ user_id: auth.userId, module_id: moduleId, variant_id: "primary" }).select("id").single();
  if (error || !data) { logSyncFailure("create practice attempt", error); return { attemptId: null, ...localOnly("remote_error") }; }
...
export async function loadRemoteProgress(): Promise<RemoteProgress | null> {
  ...
  const [moduleResult, streakResult, attemptResult, achievementResult] = await Promise.all([
    auth.client.from("user_module_progress").select("module_id, status, mastery_level, last_practised_at, inspect_practised, verify_practised, report_practised").eq("user_id", auth.userId),
    auth.client.from("user_streaks").select("current_streak, longest_streak, freezes_remaining, last_qualifying_date").eq("user_id", auth.userId).maybeSingle(),
    auth.client.from("practice_attempts").select("outcome, completed_at").eq("user_id", auth.userId).not("completed_at", "is", null).order("completed_at", { ascending: false }).limit(1),
    auth.client.from("user_achievements").select("code").eq("user_id", auth.userId),
  ]);
  if (moduleResult.error || streakResult.error || attemptResult.error || achievementResult.error) { logSyncFailure("load progress", moduleResult.error ?? streakResult.error ?? attemptResult.error ?? achievementResult.error); return null; }
...
export async function persistProgressSnapshot({ progress, moduleId, attemptId }: { progress: PracticeProgress; moduleId: ModuleId; attemptId?: string | null }): Promise<RemoteSyncResult> {
  ...
  if (attemptId) writes.push(inspectMutation("complete practice attempt", auth.client.from("practice_attempts").update({ completed_at: occurredAt, outcome: outcomeFromRetryResult(progress.lastRetryResult) }).eq("id", attemptId).eq("user_id", auth.userId)));
  const results = await Promise.all(writes);
  return results.every((result) => result.status === "synced") ? { status: "synced" } : localOnly("remote_error");
}

export async function persistPracticeEvent({ attemptId, eventType, stage, metadata = {} }: { attemptId: string | null; eventType: PracticeEventType; stage: string; metadata?: PracticeEventMetadata }): Promise<RemoteSyncResult> {
  if (!attemptId) return localOnly("no_session");
  const auth = await getAuthContext();
  if (!auth) return localOnly("no_session");
  return inspectMutation("record practice event", auth.client.from("practice_events").insert({ attempt_id: attemptId, event_type: eventType, stage, metadata_json: metadata }));
}

async function getAuthContext(): Promise<AuthContext | null> { const client = getSafeClient(); if (!client) return null; const userId = await getOrCreateAnonymousUser(client); return userId ? { client, userId } : null; }
function getSafeClient() { try { return createClient(); } catch { return null; } }
async function getOrCreateAnonymousUser(client: NonNullable<ReturnType<typeof createClient>>): Promise<string | null> {
  if (!anonymousUserPromise) anonymousUserPromise = (async () => { const { data } = await client.auth.getUser(); if (data.user?.id) return data.user.id; const { data: anonymousData, error } = await client.auth.signInAnonymously(); if (error) { logSyncFailure("create anonymous session", error); return null; } return anonymousData.user?.id ?? null; })().catch(() => null);
  return anonymousUserPromise;
}
async function ensureProfile(auth: AuthContext): Promise<RemoteSyncResult> { return inspectMutation("ensure profile", auth.client.from("profiles").upsert({ id: auth.userId, preferred_locale: "id" }, { onConflict: "id", ignoreDuplicates: true })); }
async function inspectMutation(operation: string, request: PromiseLike<QueryResult>): Promise<RemoteSyncResult> { const { error } = await request; if (!error) return { status: "synced" }; logSyncFailure(operation, error); return localOnly("remote_error"); }
function localOnly(reason: "no_session" | "remote_error"): RemoteSyncResult { return { status: "local_only", reason }; }
function logSyncFailure(operation: string, error: unknown) { const code = typeof error === "object" && error !== null && "code" in error ? String(error.code) : "unknown"; console.warn(`[Alethia] ${operation} was not synchronized (${code}).`); }
function outcomeFromRetryResult(result: RetryResult): "safe" | "unsafe" | "completed" { return result === "unsafe" || result === "safe" ? result : "completed"; }
function retryResultFromOutcome(value: unknown): RetryResult { return value === "safe" || value === "unsafe" ? value : null; }
function isMasteryState(value: unknown): value is MasteryState { return value === "not_started" || value === "familiar" || value === "skilled" || value === "needs_practice"; }
